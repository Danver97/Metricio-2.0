...
/*
* Two format for projection:
* - 'fielname': pick the field value.
* - ['field1', 'field2', ...]: build an object projecting only the listed properties.
*/
function project(json, projection) {
  if (typeof projection === 'string')
    return json[projection];
  if (Array.isArray(projection)) {
    const result = {};
    projection.forEach(p => { result[p] = json[p]; });
    return result;
  }
  return json;
}
